...
though. Modern container scheduling environments (AWS Batch or Kubernetes, for instance) are
capable of scheduling containers directly. This is conceptually cleaner, presumably scales better,
and sidesteps all sorts of issues for the deployer and developer such as configuring Docker and
...
prevents leveraging external cloud and multi-cloud compute and makes it unsuitable for
common Galaxy use cases such as large public instances and Galaxy deployments using institutional,
non-cloud storage. Even within the same cloud, a large shared file system can be expensive. Pulsar may
make the use of buckets and similar storage more tractable. Finally, Pulsar offers more
...
and staging down results do not map cleanly to cloud APIs and cannot be fully reasoned about
until job runtime. For this reason, code that needs to know how to stage Galaxy jobs up and down needs
to run in the cloud when the disk isn't shared, and Galaxy cannot do this directly. Galaxy jobs,
however, are typically executed in minimal BioContainers images designed specifically for the tool
being executed and are not appropriate for executing Galaxy code.

For this reason, the Pulsar runners that schedule containers run a Pulsar container beside the tool
container (or before and after it) to stage the job up and down and communicate with Galaxy.
...
Typically, a message queue is needed to communicate between Pulsar and Galaxy, even though the status of the job
...
it may be possible to get away without a message queue.
...
and with a Funnel server running locally using its default configuration, the configuration might look like:
