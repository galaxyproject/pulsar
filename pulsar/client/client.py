...
PULSAR_CONTAINER_IMAGE = "galaxy/pulsar-pod-staging:0.16.0"
...
class ARCLaunchMixin(BaseRemoteConfiguredJobClient):
    ...
    def _generate_executable(
        ...
        return dedent(f"""
            ...
            mkdir -p arc_staging_directory  # contains symlinks to files staged by ARC
            ...
            cp -an arc_staging_directory/. job_directory
    ...
    def _launch_arc_job(
        ...
        client = pyarcrest.arc.ARCRest.getClient(url=arc_endpoint, token=oidc_token)
    ...
    def kill(self):
        # take from the original pull request
        pass
...
class ARCPollingSequentialJobClient(BasePollingCoexecutionJobClient, ARCLaunchMixin):
    ...
    def get_status(self):
        arc_endpoint = self.destination_params["arc_url"]
        oidc_token = self.destination_params["oidc_token"]
        arc_job_id = self._arc_job_id  # injected by `PulsarARCJobRunner` when creating the client

        arc_client = pyarcrest.arc.ARCRest.getClient(url=arc_endpoint, token=oidc_token)
