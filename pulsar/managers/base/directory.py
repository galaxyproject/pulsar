...
import os
import stat
from typing import TYPE_CHECKING, Dict, List, Optional, Any, Union, IO
...
class DirectoryBaseManager(BaseManager):
    ...
    def return_code(self, job_id: str) -> Union[int, bytes]:
        return_code_str = self._read_job_file(
            job_id, JOB_FILE_RETURN_CODE, default=PULSAR_UNKNOWN_RETURN_CODE
        )
        return (
            int(return_code_str)
            if return_code_str and return_code_str != PULSAR_UNKNOWN_RETURN_CODE
            else return_code_str
        )
    ...
    def _write_return_code_if_unset(self, job_id: str, return_code: str) -> None:
        return_code_str = self._read_job_file(
            job_id, JOB_FILE_RETURN_CODE, default=PULSAR_UNKNOWN_RETURN_CODE
        )
    ...
    def _write_command_line(self, job_id: str, command_line: str) -> None:
        self._write_job_file(job_id, JOB_FILE_COMMAND_LINE, command_line)

    def _record_cancel(self, job_id: str):
        try:
    ...
    def _was_cancelled(self, job_id: str) -> Optional[bool]:
        try:
            ...
            log.info(
                "Failed to determine if job with id %s was cancelled, assuming no."
                % job_id
            )
            return False

    # TODO should the mode be wb?
    def _open_job_standard_output(self, job_id: str) -> IO:
        return self._job_directory(job_id).open_file(JOB_FILE_STANDARD_OUTPUT, "w")

    # TODO should the mode be wb?
    def _open_job_standard_error(self, job_id: str) -> IO:
        return self._job_directory(job_id).open_file(JOB_FILE_STANDARD_ERROR, "w")

    def _check_execution_with_tool_file(self, job_id: str, command_line) -> None:
        tool_id = self._tool_id(job_id)
        self._check_execution(job_id, tool_id, command_line)

    def _tool_id(self, job_id: str) -> Optional[str]:
        tool_id = None
        job_directory = self._job_directory(job_id)
    ...
    def _expand_command_line(
        ...
        if not self._is_windows:
            ...
            command_line = (
                f"{command_line}; {CAPTURE_RETURN_CODE}; echo $return_code > {rc_path};"
            )
        return command_line
    ...
    def _setup_job_file(
        ...
        script_env = self._job_template_env(
    ...
    def _tmp_dir(self, job_id: str) -> Optional[str]:
    ...
    def _prepare_dirs(self, job_id: str) -> str:
    ...
    def _job_template_env(
        ...
        job_template_env = {
            ...
            "preserve_python_environment": setup_params.get(
                "preserve_galaxy_python_environment", False
            ),
    ...
    def _write_job_script(self, job_id: str, contents) -> str:
