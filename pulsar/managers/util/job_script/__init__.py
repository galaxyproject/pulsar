...
MEMORY_STATEMENT_DEFAULT_TEMPLATE = Template(
    unicodify(resource_string(__name__, 'MEMORY_STATEMENT_TEMPLATE.sh'))
)
...
def job_script(template=DEFAULT_JOB_FILE_TEMPLATE, **kwds):
    ...
    if "memory_statement" not in kwds:
        kwds["memory_statement"] = MEMORY_STATEMENT_DEFAULT_TEMPLATE.safe_substitute(metadata_directory=metadata_directory)
