...
if TYPE_CHECKING:
    from pulsar.managers.base import (
        BaseManager,
        JobDirectory,
    )
...
class StatefulManagerProxy(ManagerProxy):
    ...
    def __proxy_status(
        ...
        "StateLiteral",
        Optional[Literal["to_complete", "to_preprocessing_failed", "to_running"]],
    ]:
        ...
        state_change: Optional[
            Literal["to_complete", "to_preprocessing_failed", "to_running"]
        ] = None
    ...
    def __status(
        ...
        if proxy_status in POSTPROCESSED_STATUSES and self.__postprocessing_pending(job_directory):
            return cast("StateLiteral", status.POSTPROCESSING)
        return proxy_status

    def __postprocessing_pending(self, job_directory: "JobDirectory") -> bool:
    ...
    def __handle_postprocessing(
        self, job_id: str, terminal_status: "StateLiteral"
    ) -> None:
        def do_postprocess():
