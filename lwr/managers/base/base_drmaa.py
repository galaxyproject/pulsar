...
class BaseDrmaaManager(ExternalBaseManager):
    ...
    def _build_template_attributes(self, job_id, command_line, requirements=[], env=[]):
        stdout_path = self._stdout_path(job_id)
        stderr_path = self._stderr_path(job_id)

        attributes = {
            "remoteCommand": self._setup_job_file(job_id, command_line, requirements=requirements, env=env),
